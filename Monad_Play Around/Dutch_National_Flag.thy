...
fun dnfp:: "nat \<Rightarrow> nat list \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat \<Rightarrow> nat list" where
"dnfp 0 s _ _ _ _ = s"|
"dnfp n s low high i mid = (if high > i then (if s!i < mid then (dnfp (n-1) (swap s i low) (Suc low) high (Suc i) mid)
                                else (if s!i > mid then (dnfp (n-1) (swap s i (high-1)) low (high-1) i mid)
                                  else (dnfp (n-1) s low high (Suc i) mid))) else s)"
...
value\<open>(\<lbrakk>xs = [0,0,2,2,1]\<rbrakk> \<Longrightarrow> (dnfp (length xs) xs 0 (length xs) 1 1 = [0,0,1,2,2]))\<close>


value\<open>(dnfp 5 [0,0,2,2,1] 0 5 1 1) = [0,0,1,2,2]\<close>
value\<open>(dnfp 8 [1,0,2,0,2,0,2,1] 0 8 1 1) = [0,0,0,1,1,2,2,2]\<close>
value\<open>sorted(dnfp 8 [1,0,2,0,2,0,2,1] 0 8 1 1)\<close>
value\<open>sorted(dnfp 7 [1,0,2,0,2,2,1] 0 7 1 1)\<close>
value\<open>sorted(dnfp 8 [1,0,2,0,2,2,1,0] 0 8 1 1)\<close>
value\<open>(dnfp 8 [1,0,2,0,2,2,1,0] 0 8 1 1)\<close>
